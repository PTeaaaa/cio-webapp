...
import { peopleData } from "@/data/tables/peopleData";
                ...
                <TableRow key={order.id}>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 overflow-hidden rounded-full">
                        <Image
                          width={40}
                          height={40}
                          src={order.user.image || "/images/default/default-avatar.jpg"}
                          alt={order.user.firstName}
                          onError={(e) =>
                            console.error((e.target as HTMLImageElement).id, "Image failed to load")
                          }
                        />
                      </div>
                      <div>
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    <Badge
                      size="sm"
                      color={
                        order.status === "ดำรงตำแหน่ง"
                          ? "success"
                          : order.status === "พ้นตำแหน่ง"
                            ? "warning"
                            : "error"
                      }
                    >
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    <div className="flex -space-x-2">
                      {order.whoLastEdited.accounts.length === 0 ? (
                        <div className="dark:border-gray-900">
                          Undefined
                        </div>
                      ) : (
                        order.whoLastEdited.accounts.slice(-1).map((whoName, index) => (
                          <div
                            key={index}
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400 hidden lg:block">
                    <a
                      href="/edit"
                      className="flex w-fit items-center justify-center gap-2 rounded-full border border-gray-300 bg-white px-3 py-3 text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200"
                    >
